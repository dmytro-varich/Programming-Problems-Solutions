...
Команда `ssh` (Secure Shell) обеспечивает безопасное сетевое соединение для удаленного управления серверами и передачи данных.

Команда `ssh-keygen` генерирует, управляет и преобразует ключи аутентификации для протокола SSH.

### Команды

```bash
# проверяем настройку SSH подключения
ssh -T git@github.com

# создаем пару SSH-ключей
ssh-keygen -t ed25519 -C "your_email@example.com" 

# подготовим новую папку
mkdir ~/github_via_ssh
cd ~/github_via_ssh

# клонируем проект по SSH протоколу в новую папку
git clone git@github.com:pragmatic-programmer-education/first_project.git

# выставить права доступа только для владельца файлов ключей
# (команда для ОС Linux)
chmod 600 id_*
```
